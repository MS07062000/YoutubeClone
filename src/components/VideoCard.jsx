...
const VideoCard = ({ video: { id: { videoId }, snippet } }) => {
  let image,imageHeight,imageWidth;
  if (snippet && snippet.thumbnails && snippet.thumbnails.high) {
    image = snippet.thumbnails.high.url;
    imageHeight= snippet.thumbnails.high.height;
    imageWidth=snippet.thumbnails.high.width;
  } else {
    ...
    <Card sx={{width:{md:'320px', xs:'100%'},boxShadow:'none',borderRadius:2}}>
      <Link to={videoId ? `/video/${videoId}` : demoVideoUrl}>
        <CardMedia image={image}
          ...
          sx={{ width: { xs: '100%', sm:'480px'}, height: '180px' }} />
